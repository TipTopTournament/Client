...
class Bracket extends React.Component {
    ...
    this.state = {
      ...
      data: {
        ...
        children: [
          {
            name: "Halbfinale1",
            children: [{ name: "Viertelfinale" }, { name: "Viertelfinale2" }],
          },
          { name: "Halbfinale2" },
        ],
      },
  ...
  getGameOfParticipant() {
    ...
    this.state.games.map((game) => {
      ...
      if (
        (PlayerID1 === MyID && GameState !== GameAlreadyPlayed) ||
  ...
  setupSingleGame(Game) {
    return {
      name:
        Game["participant1"].vorname +
        " " +
        Game.score1 +
        " vs. " +
        Game.score2 +
        " " +
        Game["participant2"].vorname,
    };
  }
  setup3Games(Game, Child1, Child2) {
    return {
      ...
      children: [
        {
          name:
            Child1["participant1"].vorname +
            " " +
            Child1.score1 +
            " vs. " +
            Child1.score2 +
            " " +
            Child1["participant2"].vorname,
        },
        {
          name:
            Child2["participant1"].vorname +
            " " +
            Child2.score1 +
            " vs. " +
            Child2.score2 +
            " " +
            Child2["participant2"].vorname,
        },
      ],
    };
  }
  mergeSubtree(Parent, LftChild, RgtChild) {
    return {
      name:
        Parent["participant1"].vorname +
        " " +
        Parent.score1 +
        " vs. " +
        Parent.score2 +
        " " +
        Parent["participant2"].vorname,
  ...
  async changeScore(gameId, tournamentCode) {
    try {
      ...
      await api.put(
        `/tournaments/${tournamentCode}/bracket/${gameId}/${this.state.manager}`,
        requestBody
      );
      ...
      alert(
        `Something went wrong during changing the score     : \n${handleError(
          error
        )}`
      );
    }
  ...
  async getData() {
    try {
      ...
      alert(
        `Something went wrong while fetching the bracket: \n${handleError(
          error
        )}`
      );
    }
        ...
        {!this.state.games ? (
          <Row>
            ...
            <Col>
              ...
              >
                Your tournament is being loaded{" "}
              </h4>
              <Spinner
                        ...
                        >
                          ...
                          <Form style={{ marginLeft: "30px" }}>
                            <Form.Group controlId="ControlInput1">
                              <Form.Label>
                                {gameData.participant1.vorname + "'s score"}
                              </Form.Label>
                              <Form.Control
                                type="TournamentName"
                                placeholder={gameData.participant1.vorname}
                                onChange={(e) => {
                            ...
                            <Form.Group controlId="ControlInput1">
                              <Form.Label>
                                {gameData.participant2.vorname + "'s score"}
                              </Form.Label>
                              <Form.Control
                                type="TournamentName"
                                placeholder={gameData.participant2.vorname}
                                onChange={(e) => {
                                  this.handleInputChange(
                                    "score2",
                                    e.target.value
                          ...
                          <Button
                            ...
                            onClick={() => {
                              this.changeScore(
                                gameData.gameId,
                                gameData.tournamentCode
                              );
                            }}
                          >
                            {"Edit Game of " +
                              gameData["participant1"].vorname +
                              " vs. " +
                              gameData["participant2"].vorname}
                          </Button>
                        </ListGroup.Item>
                      );
                    })}
